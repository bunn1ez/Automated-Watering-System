...
int relaySignal = 5; 
int THRESHOLD = 320; // This is +40 over the value 280 for error which represents the minimum value it takes for the soil to get moist and turn the pump off
...
void loop() {
  ...
  // Check the water sensor 7 times with 40 second intervals on
  for (i=0; i < 7; i++){
    delay(800);
    int value = analogRead(waterSensor);

    // If the soil is dry, turn on the pump for 40 seconds intervals
    if (value > THRESHOLD) {
  ...
  while (millis() - startTime < 3600000UL * 36) {
    ...
    lcd.setCursor(0, 1);
    lcd.print("Watering in:"); // Prints to the lcd that the watering cycle will repeat in # amount of time
    lcd.setCursor(13, 1);
